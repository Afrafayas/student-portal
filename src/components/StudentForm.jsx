...
//   const existingStudent = isEdit ? students.find(s => s.id ===studentId) : null
...
function StudentForm() {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEdit) {
      // Find the existing student from Redux or create from current data
      const existingStudent = students.find(s => s.id === Number(id))
      ...
      const updatedStudent = {
        ...
        username: existingStudent?.username || formData.name.toLowerCase().replace(/\s+/g, '.'),
        website: existingStudent?.website || 'www.example.com',
        company: existingStudent?.company || { name: 'Student Company' },
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32">
        ...
        <p className="mt-6 text-lg text-gray-600 animate-pulse">Loading student data...</p>
      </div>
    )
  }
